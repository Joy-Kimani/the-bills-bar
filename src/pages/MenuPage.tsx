...
const MenuPage: React.FC = () => {
  ...
  const proceedCheckout = () => setShowCheckout(true);
  ...
  return (
    <>
      ...
      {/* Restructured Container:
          1. flex flex-col min-h-screen: Ensures page takes full height.
          2. selection:bg-[#d4af37]/30: Custom highlight color.
      */}
      <div className="flex flex-col min-h-screen bg-[#0a0a0a] text-[#f5f5f5] font-sans selection:bg-[#d4af37]/30">
        
        {/* flex-grow pushes everything below it (the footer) to the bottom */}
        <div className="flex-grow pb-32 md:pb-40">
          <Navbar />

          {/* HEADER */}
          <header className="pt-10 md:pt-16 pb-6 px-4 text-center">
            <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              className="text-[#d4af37] text-[9px] md:text-[11px] uppercase tracking-[0.4em] mb-2 block">
              Premium Experience
            </motion.span>
            <h1 className="text-3xl sm:text-4xl md:text-6xl font-serif uppercase tracking-widest font-bold">
              Bills Lounge
            </h1>
          </header>
            ...
            <div className="flex w-full max-w-xs bg-white/5 border border-white/10 rounded-full p-1 backdrop-blur-sm">
              {(["KITCHEN", "BAR"] as OrderMode[]).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  className={`flex-1 py-2.5 text-[10px] md:text-[11px] uppercase tracking-widest rounded-full transition-all duration-300 ${
                    mode === m ? "bg-[#D4AF37] text-black font-bold shadow-lg shadow-[#D4AF37]/20" : "opacity-40 hover:opacity-100"
                  }`}>
                  {m}
                </button>
              ))}
            ...
            <div className="flex gap-3 px-4 overflow-x-auto no-scrollbar sm:justify-center">
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  className={`whitespace-nowrap px-5 py-2 rounded-full text-[9px] md:text-[10px] uppercase tracking-widest border transition-all duration-300 ${
                    activeCategory === cat ? "border-[#d4af37] text-[#d4af37] bg-[#d4af37]/10" : "border-white/10 opacity-40 hover:opacity-70"
                  }`}>
                  {cat}
                </button>
              ))}
          ...
          {orderItems.length > 0 && (
            <motion.div initial={{ y: 100, x: "-50%" }} animate={{ y: 0, x: "-50%" }} exit={{ y: 100, x: "-50%" }}
              ...
              <div className="flex gap-2">
                <button onClick={() => setOrderItems([])} className="p-3 rounded-xl bg-red-500/10 text-red-500">
                  <Trash2 size={18} />
                </button>
              ...
              <motion.div className="relative bg-[#1a1a1a] w-full max-w-md rounded-[2rem] p-8 border border-white/10 shadow-2xl">
                  ...
                  {orderItems.map(item => (
                    <div key={item.id} className="flex justify-between items-center text-sm border-b border-white/5 pb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => setOrderItems(prev => prev.map(i => i.id === item.id ? { ...i, quantity: Math.max(1, i.quantity - 1) } : i))}
                          className="p-1 rounded-full border border-white/20 hover:bg-white/10">–</button>
                        <span>{item.quantity}</span>
                        <button onClick={() => setOrderItems(prev => prev.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i))}
                          className="p-1 rounded-full border border-white/20 hover:bg-white/10">+</button>
                        <span className="ml-2">KES {(item.price * item.quantity).toLocaleString()}</span>
                        <button onClick={() => setOrderItems(prev => prev.filter(i => i.id !== item.id))} className="ml-2 p-1 rounded-full hover:bg-red-500/20">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={confirmOrder} disabled={processing}
                    ...
                    {processing ? <Loader2 className="animate-spin" size={16} /> : "Confirm Order"}
                  </button>
                </div>
              </motion.div>
            </div>
        ...
        {/* Footer is now inside the flex column, forced to the bottom by the flex-grow div above */}
        <Footer />
      </div>
